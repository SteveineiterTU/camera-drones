...
then
  ...
  ${TMUX} send-keys -t ${SESSION_NAME}:1.0 'rosrun octomap_path_planner octomap_path_planner_ros_node trajectory_visualization --runtime 3 --planner RRTStar -o WeightedLengthAndClearanceCombo -f planner_trajectory.txt --info 2 --octomap /home/student/camera-drones/catkin_ws/src/octomap_path_planner/maps/dronespace_synthetic.bt' C-m
  #    Octomap server 
  ${TMUX} send-keys -t ${SESSION_NAME}:1.1 'source $HOME/.bashrc' C-m
  ${TMUX} send-keys -t ${SESSION_NAME}:1.1 'roslaunch octomap_path_planner octomap_mapping_a2.launch octomap:=$HOME/camera-drones/catkin_ws/src/octomap_path_planner/maps/dronespace_synthetic.bt --wait' C-m
  #    Trajectory Sampler
  ...
  ${TMUX} send-keys -t ${SESSION_NAME}:2.0 'rostopic pub /octomap_path_planner/current_position geometry_msgs/Point "x: -1.75
y: 0
z: 1.50" --once' C-m
  sleep 4s
  ${TMUX} send-keys -t ${SESSION_NAME}:2.2 'source $HOME/.bashrc' C-m
  ${TMUX} send-keys -t ${SESSION_NAME}:2.2 'rostopic pub /octomap_path_planner/goal_position geometry_msgs/Point "x: -0.03
y: 0.13
z: 0.35" --once' C-m
  sleep 4s
