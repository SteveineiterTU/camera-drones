...
then
  ...
  ${TMUX} send-keys -t ${SESSION_NAME}:1.6 'source $HOME/.bashrc' C-m
  ${TMUX} send-keys -t ${SESSION_NAME}:1.6 'rostopic pub /octomap_path_planner/current_position geometry_msgs/Point "x: 14	
y: 8 
z: 5" --once' C-m
  sleep 4s
  # ${TMUX} send-keys -t ${SESSION_NAME}:1.6 'source $HOME/.bashrc' C-m
  # ${TMUX} send-keys -t ${SESSION_NAME}:1.6 'rostopic pub /octomap_path_planner/goal_position geometry_msgs/Point "x: 28
y: 8
z: 1" --once' C-m
  sleep 4s
  # Trajectory Sampler
  ${TMUX} send-keys -t ${SESSION_NAME}:1.1 'source $HOME/.bashrc' C-m
  ${TMUX} send-keys -t ${SESSION_NAME}:1.1 'roslaunch trajectory_planner trajectory_planner.launch dt:=0.01' C-m  
